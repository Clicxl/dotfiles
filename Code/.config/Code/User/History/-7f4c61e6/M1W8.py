...
class GUI:
    def __init__(self,size,file) -> None:
        ...
        self.convert = Converter("image.jpg", 'jpg', 5)
    def img_place(self):
    ...
    def text_frame(self):
        self.StringVar = ttk.StringVar()
        textframe = ttk.Frame(master=self.app)
        label = ttk.Label(master=textframe,text=self.StringVar.get())
        label.pack()

        textframe.place(x= 3* self.size[1]// 4 + 50, y = 20)
        self.convert.update()
        self.StringVar.set(self.convert.grid)
        ic(self.convert.grid)
