...
class Converter:
    ...
    def ascii(self):
        ...
        self.file_name_changer()
        with open(f"{self.file_name}.txt", "w") as file:
            for row in self.grid:
                file.write("".join(row) + "\n")
        remove(f"resized.{self.type}")

    def file_name_changer(self):
        self.file_name = self.file_destiantion.split("/")[-1]

    def update(self):
        self.pil_img()
        self.scale_img()
        self.ascii()
        print(f"Your Image is Done and saved in '{self.file_name}.txt'")
