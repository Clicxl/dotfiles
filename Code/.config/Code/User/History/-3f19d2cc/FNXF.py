    ...
    def debugger(self, defbug_list):
        for index, name in enumerate(defbug_list):
            self.game.render_font(
                str(name), COLORS["white"], vec(10, 25 * index), self.game.font, False
            )
...
class Scene(State):
    ...
    def create_scene(self):
        ...
        if "blocks" in layers:
            for x, y, surf in self.tmx_data.get_layer_by_name("blocks").tiles():
                Wall(
                    [self.block_sprites, self.draw_sprites],
    ...
    def draw(self, screen):
        ...
        self.debugger([str(f"FPS: {self.game.clock.get_fps():.2f}"),f"angle: {self.player.get_direction():.0f}"])
