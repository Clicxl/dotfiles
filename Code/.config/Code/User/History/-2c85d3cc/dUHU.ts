import { modResponseType, modType } from "./types";

async function getData(Baseurl: string, Name: string): Promise<object> {
  const mod_ids: Object<modType> = {
    author: "",
    categories: [],
    client_side: "",
    description: "",
  ...
  try {
    ...
    mods.forEach((mod: modResponseType) => {
      // console.log(mod);
...
async function getMods(Baseurl: string, ID: string): Promise<void> {
