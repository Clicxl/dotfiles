...
function findMachtes(wordInput, cities) {
    const filtCities = cities.filter(place => {
    ...
    return filtCities.sort((a, b) => {
        return a.rank > b.rank ? 1 : -1 || a.population > b.population? 1: -1;
				});
}
...
function dispMatches() {
  ...
  const html = matchArr.map(place => {
    ...
    return `
    <li>
    <span class='name'>${cityName}, ${stateName}: ${place.rank}</span>
