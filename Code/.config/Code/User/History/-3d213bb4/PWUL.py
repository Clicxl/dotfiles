...
class GUI:
    ...
    def main_word_frame(self,width,height,font:str):
        ...
        self.Word = ttk.Label(master=self.main_word,text="WORD",
                            font=font)

        self.letStr1 = ttk.StringVar(value="text")
        self.letter1 = ttk.Entry(master=self.main_word, width=width,
                            font=font,textvariable=self.letStr1)
        self.letStr1.trace_add(
            'write', lambda var, index, mode: self.logic.one_word(var=var, index=index, mode=mode,strvar=self.letStr1))

        self.letStr2 = ttk.StringVar()
        self.letter2 = ttk.Entry(master=self.main_word,width=width,
                            font=font,textvariable=self.letStr2)
        self.letStr3 = ttk.StringVar()
        self.letter3 = ttk.Entry(master=self.main_word,width=width,
                            font=font,textvariable=self.letStr3)
        self.letStr4 = ttk.StringVar()
        self.letter4 = ttk.Entry(master=self.main_word,width=width,
                            font=font,textvariable=self.letStr4)
        self.letStr5 = ttk.StringVar()
        self.letter5 = ttk.Entry(master=self.main_word,width=width,
                            font=font,textvariable=self.letStr5,)

        self.Word.pack(side=self.side,padx=10)
    ...
    def good_words_frame(self):
        ...
        self.good_word = ttk.Label(
            master=self.good,
            text='Good Letters',
            font="Segoe_UI 12 bold")
        letters = ttk.StringVar()
        letters_entry = ttk.Entry(master=self.good, font="Segoe_UI",textvariable=letters)
    ...
    def search_frame(self):
        ...
        self.search_button = ttk.Button(master=self.search,text="Search",width=15,)
...
class GameLogic:
    ...
    def one_word(self, var, index, mode,strvar):
        self.strvar = strvar
        self.get = self.strvar.get()

        if len(self.get) > 1:
            self.strvar.set(self.get[0].)
