...
element_emailInput.addEventListener("focusout", (event) => {
  ...
  let emailError: HTMLElement | null = document.querySelector("emailerror");
...
element_passwordInput.addEventListener("input", (event) => {
  ...
  let passwordError:HTMLElement | null = document.getElementById("passworderror"); // Does an error already exist?
 ...
 */
function sendForm(username:string, email:string, password) {
  let OK = false;
  let config = {
    method: "POST",
