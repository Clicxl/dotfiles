...
def todayChal():
    challenges: list = []
    for i in range(
        0, int(pyautogui.prompt(text="Enter No of Challenges: ", title="Post"))
    ):  # type: ignore
        challenges.append(
            str(pyautogui.prompt(text=f"Enter Challenge {i+1}: ", title="Challenges"))
        )  # type: ignore
    return challenges

Challenges: list = todayChal()


class Base:
    todayChallenges: list = Challenges
