...
import {
  element_loginLink,
  element_loginText,
  element_createaccountLink,
  element_createaccountText,
  element_verifyErrorElement,
  element_verifyConfirmElement,
  element_sendEmail,
} from "./CONSTANTS";
...
element_sendEmail.addEventListener("click", () => {
  resendConfirmEmail();
});
...
const splitHREF: string[] = window.location.href.split("/");
const member: string = splitHREF[splitHREF.length - 1];
...
function refreshAndUpdateNav() {
  ...
  fetch("/refresh")
    ...
    .then((result) => {
      if (OK) {
        // Refresh token (from cookie) accepted! Receiving new access token + member name
        console.log("Logged in");
        // token = result.accessToken;
        token = getCookieValue("token"); // Cookie expires in 60s

        loadMemberData(result.member.toLowerCase() as string);

        // Change navigation links...
        element_loginLink.setAttribute(
          "href",
          `/member/${result.member.toLowerCase()}`
        );
        ...
        // Unauthorized, don't change any navigation links
...
function loadMemberData(loggedInAs?: string): JSON | void {
  let config: formConfigType = {
    // Send with our access token
    method: "GET",
    headers: {
  ...
  fetch(`/member/${member}/data`, config)
    .then((response) => {
      ...
      else if (response.status === 500) window.location.href = "/500";
    ...
    .then((result) => {
      ...
      const element_memberName = document.getElementById(
        "member-name"
      ) as HTMLElement | null;
      const eloElement = document.getElementById("elo") as HTMLElement | null;
      const joinedElement = document.getElementById(
        "joined"
      ) as HTMLElement | null;
      const seenElement = document.getElementById("seen") as HTMLElement | null;

      if (element_memberName && eloElement && joinedElement && seenElement) {
        element_memberName.textContent = result.username;
        eloElement.textContent = result.elo.toString();
      ...
      if (loggedInAs === result.username.toLowerCase()) {
        ...
        if (result.verified === true)
          revealElement(element_verifyConfirmElement);
        // They just confirmed, tell them it was a success!
        else if (result.verified === false)
          revealElement(element_verifyErrorElement);

        // Display email
        revealElement(element_showAccountInfo);
        // Display remove button
        revealElement(element_deleteAccount);
        element_deleteAccount.addEventListener("click", () =>
          removeAccount(true)
        );
...
async function removeAccount(confirmation) {
  if (
    !confirmation ||
    confirm(
      "Are you sure you want to delete your account? This CANNOT be undone! Click OK to enter your password."
    )
  ) {
...
function resendConfirmEmail() {
  ...
  fetch(`/member/${member}/send-email`, config)
    ...
    .then((result) => {
      // Email was resent! Reload the page
...
function recalcUsernameSize() {
  // Change username text size depending on character count
  const memberElementPadding = parseInt(
    window
      .getComputedStyle(element_member, null)
      .getPropertyValue("padding-left"),
    10
  ); // parseInt() converts px to number
