...
element_usernameInput?.addEventListener("focusout", async () => {
  // Check username availability...
  if (element_usernameInput?.value.length === 0 || usernameHasError) return;

  if (element_usernameInput instanceof HTMLElement) {
    await fetch(`/createaccount/username/${element_usernameInput.value}`)
      ...
      .then((result) => {
        ...
        createErrorElement("usernameerror", "usernameinputline");
        // Change input box to red outline
        element_usernameInputstyle.outline = "solid 1px red";
        // Reset variable because it now exists.
        usernameError = document.getElementById("usernameerror");
