...
const element_forgot: HTMLElement = document.getElementById("forgot") as HTMLElement;
...
const sendLogin = (username:string, password:string) => {
  ...
  fetch("/auth", config)
    ...
    .then((result) => {
      if (OK) {
        // Username & password accepted! Handle our access token
        // const token = getCookieValue('token')

        // Check for a redirectTo query parameter, and if it exists, use it
        if  (typeof getQueryParam("redirectTo")  === "string"){

        }
        const redirectTo:string = getQueryParam("redirectTo") as string;
        window.location.href = redirectTo;
        else window.location.href = `/member/${username.toLowerCase()}`;
      } else {
        // Unauthorized, create error with the message contained in response body
        if (!loginErrorElement) {
